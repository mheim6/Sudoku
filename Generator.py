...
def check_number(s, row, col):
#	Returns True if a given cell is valid in a Sudoku puzzle, and False if not. A cell is valid if the number in that cell is not present
...
#	set it to blank and return false.
	...
	for i in range(9):
		s[row][col] = temp[i]
		if check_number(s, row, col):
			if implement_board(s, row, col+1):
				return True
	s[row][col] = 0
	return False
...
#	"""
	...
	while indices:
		...
		pySudoku.first_try(copy_s)

		for line in copy_s:
			if 0 in line:
				num_solutions = DFS_solve(copy_s, 0, 0)
				# Not a puzzle with a unique solution, so ignore the last insertion
				if num_solutions > 1:
					s[row][col] = temp
...
def main():
	...
	for _ in range(user_input):
		...
		implement_board(s, 0, 0)
		reduce_sudoku(s, difficulty)
		output = format_string(s)
		f.write(output)
		#output.write("Solution.txt")
		#print(output)
